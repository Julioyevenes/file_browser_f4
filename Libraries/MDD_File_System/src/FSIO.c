...
#if defined (__C30__) || defined (__PIC32MX__) || defined (STM32F4XX)
...
void CacheTime (void)
{
...
#if defined (__PIC32MX__) || defined (STM32F4XX)   // Added support for PIC32. -Bud (3/4/2008)
	...
	{
#if defined(STM32F4XX)
	   t0 = (uint32_t)(RTC->TR & ((uint32_t)0x007F7F7F));
	   t1 = (uint32_t)(RTC->TR & ((uint32_t)0x007F7F7F));
#else
		t0=RTCTIME;
		t1=RTCTIME;
#endif
	...
	{
#if defined(STM32F4XX)
	   d0 = (uint32_t)(RTC->DR & ((uint32_t)0x00FFFF3F));
	   d1 = (uint32_t)(RTC->DR & ((uint32_t)0x00FFFF3F));
#else
		d0=RTCDATE;
		d1=RTCDATE;
#endif
