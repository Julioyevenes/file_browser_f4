...
#if defined(STM32F4XX)
/**
  * @brief   SCSI MSD Host Global Variables
  */
...
//******************************************************************************
#if defined(STM32F4XX)
	#define USBTasks() USBH_Process(&USB_OTG_Core, &USB_Host);
#else
#if !defined( USBTasks )
    #define USBTasks()                  \
        {                               \
...
MEDIA_INFORMATION * USBHostMSDSCSIMediaInitialize( void )
{
    ...
    while (attempts != 0)
    {
        ...
        do
        {
           errorCode = USBH_MSC_ReadCapacity10(&USB_OTG_Core);
           USBTasks();
        }
        while(errorCode == USBH_MSC_BUSY);
#else
        ...
        if (!errorCode)
        {
            ...
            #endif
#if defined(STM32F4XX)
            mediaInformation.sectorSize = (WORD) USBH_MSC_Param.MSPageLength;
#else
        ...
        else
        {
           ...
           do
           {
              errorCode = USBH_MSC_RequestSense(&USB_OTG_Core);
              USBTasks();
           }
           while(errorCode == USBH_MSC_BUSY);
#else
...
BYTE USBHostMSDSCSISectorRead( DWORD sectorAddress, BYTE *dataBuffer )
{
    ...
    if (deviceAddress == 0)
    {
        ...
        do
        {
           errorCode = USBH_MSC_Read10(&USB_OTG_Core, dataBuffer, sectorAddress, mediaInformation.sectorSize);
           USBTasks();
...
BYTE USBHostMSDSCSISectorWrite( DWORD sectorAddress, BYTE *dataBuffer, BYTE allowWriteToZero )
{
...
#if defined(STM32F4XX)
    if ((!HCD_IsDeviceConnected(&USB_OTG_Core)) && (allowWriteToZero == FALSE))
    {
        return FALSE;
    }
#else
    ...
    if ((sectorAddress == 0) && (allowWriteToZero == FALSE))
    {
        ...
        do
        {
           errorCode = USBH_MSC_Write10(&USB_OTG_Core, dataBuffer, sectorAddress, mediaInformation.sectorSize);
           USBTasks();
...
BOOL _USBHostMSDSCSI_TestUnitReady( void )
{
    ...
    while (unitReadyCount < 5)
    {
        ...
        do
        {
           errorCode = USBH_MSC_TestUnitReady(&USB_OTG_Core);
           USBTasks();
        }
        while(errorCode == USBH_MSC_BUSY);
#else
